...
{
    /* this radix sort function is designed for large n
     * if n is small, it is too costy to allocate large
     * ammount of memory.
     * this radix sort is also good for large nbits.
     * radix sort need to copy the ary[] several time
     * during sort, so it costs more time and space than
     * counting sort. The advantage of radix sort is that
     * it can handle large nbits, while counting sort can
     * only handle nbits of 16 due to the limit of memory.
     */
    unsigned long i, j, size, itr;
    unsigned long * count;
    unsigned long * pos;
    unsigned long * temp;
    /* determine the size of counting bin */
    if( nbits > 16 )
        size = 16;
    else
        size = nbits;
    /* determine the times of iteration */
    if( nbits % size )
        itr = nbits / size + 1;
    else
        itr = nbits / size;
    ...
    for ( i = 0;  i < itr; i++)
    {
        count = calloc( 1 << size, sizeof( unsigned long ) );
        pos = calloc( 1 << size, sizeof( unsigned long ) );
        temp = calloc( n, sizeof( unsigned long ) );
        for ( j = 0; j < n; j++ )
        {
            temp[ j ] = ary[ j ];
            count[ ( ary[ j ] >> ( size * i ) ) & ( ( 1 << size ) - 1 ) ]++;
        }
        for ( j = 1; j < 1 << size; j++ ) {
        ...
        for ( j = 0; j < n; j++ )
        {
            ary[ pos[ ( temp[ j ] >> ( size * i ) ) & ( ( 1 << size ) - 1 ) ]++ ] = temp[ j ];
        }
        free( count );
        free( pos );
        free( temp );
				     ...
				     unsigned long n, unsigned long nbits)
{
    ...
    count = calloc( 1 << nbits, sizeof( unsigned long ) );
    for ( i = 0; i < n; i++ ) {
        count[ ary[ i ] ]++;
    }
...
/*
 *  helper function of myQSort
 */
static unsigned long rPartition( unsigned long ary[], unsigned long start, unsigned long end )
{
    unsigned long index, temp, i;
    /*time_t t;
...
/*
 *  helper function of do_quicksort
 */
static void myQSort( unsigned long ary[], unsigned long start, unsigned long end )
{
    if( start < end )
    {
        unsigned long index = rPartition( ary, start, end );
        if( start < index)
            myQSort( ary, start, index - 1 );
        myQSort( ary, index + 1, end );
    }
}
